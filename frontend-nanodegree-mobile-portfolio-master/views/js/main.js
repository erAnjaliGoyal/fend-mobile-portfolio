...
function changeSliderLabel(size) {
  ...
  function changePizzaSizes(size) {
    var ContainerValue=document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(ContainerValue[0], size);
    var newwidth = (ContainerValue[0].offsetWidth + dx) + 'px';
    for (var i = 0; i < ContainerValue.length; i++) {
        ContainerValue[i].style.width = newwidth;
    }
  }
...
var resizePizzas = function(size) {
  window.performance.mark("mark_start_resize");  
    ...
    var timeResize = window.performance.getEntriesByName("measure_pizza_resize");
    console.log("Time to resize pizzas: " + timeResize[timeResize.length - 1].duration + "ms");
  };
